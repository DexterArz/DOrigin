...
HYPRLOCK_WALLPAPER="${XDG_CONFIG_HOME:-$HOME/.config}/hypr/hyprlock/wallpaper"
...
CURRENT_WALLPAPER=$(
    grep "^$CURRENT_THEME:" "$WALLPAPER_STATE" 2>/dev/null |
    cut -d':' -f2-
)
...
# Hyprlock can always reference this fixed path.
mkdir -p "$(dirname "$HYPRLOCK_WALLPAPER")"

ln -sfn "$selected_path" "$HYPRLOCK_WALLPAPER"
